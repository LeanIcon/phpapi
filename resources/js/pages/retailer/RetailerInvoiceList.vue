            ...
            <tbody>
              <tr v-for="(order) in processed_orders" :key="order.id">
                ...
                <td>
                  <span
                    class="badge"
                    :class="{'badge-info' : order.status == 'pending', 'badge-success' : order.status == 'processed', 'badge-danger' : order.status == 'cancelled'}"
                  >{{order.status}}</span>
                </td>
                <td>
    ...
    async loadPurchaseOrders(url = "retailer_purchase_order") {
      ...
      await axios
        ...
        .then(({ data }) => {
          ...
          this.processed_orders = this.purchase_orders.filter(function (purchase_order) {
